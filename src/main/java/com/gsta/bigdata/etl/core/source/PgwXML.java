...
public class PgwXML extends AbstractSourceMetaData {
	...
	public ETLData parseLine(String line, Set<String> invalidRecords)
		...
		if (line.contains(beginContent)) {
			...
			try {
				return getContent(xml,invalidRecords);
	...
	private ETLData getContent(String xml,Set<String> invalidRecords) {
		if (xml == null) {
			return null;
		}
			...
			if(this.fieldIds != null && this.fieldIds.size() >0){
				for (String fieldName : data.getFieldNames()) {
					...
					if (this.fieldIds.contains(fieldName)) {
						...
						super.fieldValidate(field, fieldValue, xml, invalidRecords);
					}
				} 
			}
