...
public class SgwXML extends AbstractSourceMetaData {
	...
	private static final String TAG_NETWORKINITIATION = "networkInitiation";
	...
	public ETLData parseLine(String line, Set<String> invalidRecords)
		...
		if(line.contains(benginListService) && line.contains(endListService)){
			this.listServiceFlag = false;
		}else if (line.contains(benginListService)) {
	...
	private ETLData getContent(List<String> xmlList, Set<String> invalidRecords) {
		...
		try {
			...
			String networkInitiation = this.getValueFromList(xmlList, TAG_NETWORKINITIATION);
			data.addData(TAG_NETWORKINITIATION, networkInitiation);
