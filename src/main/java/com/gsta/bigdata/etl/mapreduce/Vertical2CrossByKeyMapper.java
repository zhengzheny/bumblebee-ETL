...
public class Vertical2CrossByKeyMapper extends Vertical2CrossByFileMapper {
	private String lastMasterKey = "";
	...
	protected void map(Object key, Text value_,
		...
		ETLData data = null;
		try {
			data = super.process.parseLine(value.toString(),
					super.invalidRecords);
			// add filename to output if you need
			if (super.outputSourceFileName) {
				data.addData(Constants.OUTPUT_FIELD_FILE_NAME,
			...
			if (super.errorRecords.size() >= super.errorRecordThreshold) {
				// writeFiles(this.errorOutput,
				// this.errorRecords,this.errorFileName);
				super.writeFiles(super.errorPath,
						Constants.OUTPUT_ERROR_FILE_PREFIX, errorRecords);
			}
			...
			if (super.invalidRecords.size() >= super.errorRecordThreshold) {
				// writeFiles(this.invalidOutput,
				// this.invalidRecords,this.invalidFileName);
				super.writeFiles(super.errorPath,
						Constants.OUTPUT_INVALID_FILE_PREFIX, invalidRecords);
			}
			return;
		}
		...
		if (data != null) {
			...
			if (!this.lastMasterKey.equals("")
					&& !this.lastMasterKey.equals(dataKey)) {
				ETLData etlData = new ETLData();
				etlData.addData(this.crossDatas.get(lastMasterKey));
				...
				this.crossDatas.remove(this.lastMasterKey);
				this.lastMasterKey = dataKey;
			}
		}
	}
